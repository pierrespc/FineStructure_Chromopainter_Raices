...
pref="Genotipos_Raices.Plink.Autosomal.HGDP_1KG_SGDP_REDUCED.Filtered.pruned"
listKNUM=c(2:13)
OrderIND="None"
...
regions<-read.table(paste(folder,"/StartFilteredData/FinalColors.tsv",sep=""),stringsAsFactors=F,header=T,comment.char = "@",sep="\t")
...
for(KNUM in listKNUM){
  ...
  getFile=list.files(pattern=paste(pref,".*.",KNUM,".Q",sep=""))
  getFile=getFile[which(getFile %in% paste(paste(pref,".rep",c(1:10),".",KNUM,".Q",sep="")))]
...
#	outfile<-paste("chr1.1KG.PopArg.pruned.K",KNUM,".pdf",sep="")
  ...
  atRegion<-c()
  ITER = 0
  for(reg in c("Central Native American","South Native American","AfroAmerican","Admixed American")){
    temp<-out[ out$Region == reg,]
    Population=temp$Population[1]
    ITER=ITER+1
  ...
  #axis(1,at=atRegion,label=names(atRegion),cex.axis=6,tick=F,line=4)
  text(x=(numInds+separPop*(numPops-numRegions)+numRegions*(separ))*1.05,y=0.5,labels = "Native Americans\nAfro-Americans\nand Hispano Americans",cex=10)
