...
commandCP="/pasteur/zeus/projets/p02/Hotpaleo/pierre/Scripts/ChromoPainterv2/ChromoPainterv2 "
...
do
	...
	echo -e $cluster"\t"$nCluster  >> stage5.DonorList
	echo -e pop$iterClu"\t"$nCluster  >> stage5.DonorList2
...
listAdmPop=$(awk -v OFS="\t" '{if($3==0)print $2}'  $folder/fineStructure/Inputs/Genotipos_Raices.Plink.Autosomal.HGDP_1KG_SGDP_REDUCED.Filtered.MAF0.0000001.GENO1.MIND1_STEP1.ids | sort  | uniq)
...
do
	...
	awk -v p=$popTest -v OFS="\t" '{if($2==p)print $1,$2,1}' $folder/fineStructure/Inputs/Genotipos_Raices.Plink.Autosomal.HGDP_1KG_SGDP_REDUCED.Filtered.MAF0.0000001.GENO1.MIND1_STEP1.ids >> $popTest/stage5.$popTest.ids
	awk -v p=$popTest -v OFS="\t" '{if($2==p)print $1,$2,1}' $folder/fineStructure/Inputs/Genotipos_Raices.Plink.Autosomal.HGDP_1KG_SGDP_REDUCED.Filtered.MAF0.0000001.GENO1.MIND1_STEP1.ids >> $popTest/stage5.$popTest.ids2
done
	...
	then
		...
		do
			...
			then
				...
				numLineExpect=$(wc -l $popTest/stage5.$popTest.ids | awk '{print $1}')
				let ' numLineExpect = ( numLineExpect * 2 ) + 3 '
				if [[ $formatCPmanual == "T" ]]
				then
				...
				then
					...
					let ' nIndTestPop = nIndTestPop + totalDonorInd '
					if [[ $formatCPmanual == "T" ]]
					then
			                	echo $nTotalDonor > $popTest/chr$i/stage5.$popTest.chr$i.phase
					else
						let ' nIndTestPop = nIndTestPop * 2 '
					fi
					
		                        echo $nIndTestPop >> $popTest/chr$i/stage5.$popTest.chr$i.phase
			                sed -n 2,3p $folder/fineStructure/Inputs/Genotipos_Raices.Plink.Autosomal.HGDP_1KG_SGDP_REDUCED.Filtered.MAF0.0000001.GENO1.MIND1.chr$i"_alignedRef_phased.phase" >> $popTest/chr$i/stage5.$popTest.chr$i.phase
					...
					do
						...
						line=$(awk -v id=$id '{if($1==id)print NR}' $folder/fineStructure/Inputs/Genotipos_Raices.Plink.Autosomal.HGDP_1KG_SGDP_REDUCED.Filtered.MAF0.0000001.GENO1.MIND1_ALL.ids)
						...
						awk -v line1=$line1 -v line2=$line2 '{if(NR==line1 || NR==line2)print $0}' $folder/fineStructure/Inputs/Genotipos_Raices.Plink.Autosomal.HGDP_1KG_SGDP_REDUCED.Filtered.MAF0.0000001.GENO1.MIND1.chr$i"_alignedRef_phased.phase" >> $popTest/chr$i/stage5.$popTest.chr$i.phase
				
					done < $popTest/stage5.$popTest.ids
				fi
				sbatch -J chr$i.$popTest -o logs/stage5.$popTest.chr$i.o -e logs/stage5.$popTest.chr$i.e --mem=4G --cpus-per-task=3 --time=6:00:00 \
					--wrap "$commandCP \
